...
public class CommitLog {
    ...
    /**
     * 存储每个topic-queue-id 与 queue 偏移量offset
     * 个人理解是每个message quque 对应的 consumer queue 消息到哪里了
     */
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
             ...
             *
             计算方式	长度
             broker存储的msgId:offsetMsgId	Broker存储时生成	Hex(storeHostBytes, wroteOffset)	32
             client: msgId	Client发送消息时生成	Hex(进程编号, IP, ClassLoader, startTime, currentTime, 自增序列)
             */

            // 根据存储broker机器的host + commitLog文件的物理位置offset 生成
